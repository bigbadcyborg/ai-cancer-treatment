...
#include <cmath>
#include <iterator>
//#include "../enumSerialization/enumSerialization.h"
...
// Prototype(s)
vector<ClauseType>* convertVariableListToValidClauseVector(const string &str, const RuleBase*);
string toLower(string & str);
vector<vector<ClauseType>>* generateSubsets(const vector<ClauseType>&, const RuleBase*);
bool checkRuleSatisfaction(const vector<ClauseType>&, const Rule&, const RuleBase*);


int main() {
	cout.sync_with_stdio(false); // Disable syncing with C stdio
	cout << std::unitbuf;        // Forces flushing after every output operation
	...
	ConclusionType conclusionTypeFound;
    for (const auto& subset : *subsetsPtr) {
        for (const auto& rule : *(myRuleBase->ruleList)) {
            if (checkRuleSatisfaction(subset, rule, myRuleBase)) {
				...
				conclusionTypeFound = rule.conclusion;
    ...
    // Return the ConclusionType to forwardChaining via piping
	// KnowledgeBase should see these changes with FactBase
	if ( conclusionFound )
		EnumSerialization::serializeAndPipe( conclusionTypeFound );
