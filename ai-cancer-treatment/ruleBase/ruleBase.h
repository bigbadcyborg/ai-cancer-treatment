...
// Prototype(s)
string ClauseTypeToString(const ClauseType &, const unordered_map<ClauseType, string>*);
...
// RuleBase Class ( uses the Global Static FactBase )
class RuleBase {
public:
	...
	// RUNTIME FUNCTIONS:
	// each use templated double ended queue (MyDeque) for dynamic run-time handling of
    // new ClauseType and ConclusionType:
	void createRule(Rule);
	void destroyRule();
	void createClauseType(string);
	void destroyClauseType(string);
	void createConclusionType(string);
	void destroyConclusionType(string);
	void updateRuleBaseFromFactBase();
};
